...
- Tienda de moda femenina en Cartagena; pedidos por WhatsApp. Vite + React + TypeScript, Tailwind + shadcn/ui, Zustand (carrito).
- Catálogo en `src/data/products.ts`, fotos en `public/products/`, logo en `public/brand/logo.jpg`.
- Se publica con GitHub Actions (`.github/workflows/deploy.yml`): `npm run build` → `dist`. Verifica que `npm run build` pase antes de subir.
- La marca de la tienda manda sobre la lista de prohibidos de diseño.
